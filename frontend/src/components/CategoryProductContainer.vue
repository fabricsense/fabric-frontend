<template>
  <div class="border border-gray-200 rounded-lg bg-white">
    ...
    >
      ...
      <div class="space-y-2">
        <div v-if="products.length === 0" class="px-4 py-6 text-center text-gray-500 text-sm">
          {{ __('Please add a product') }}
        </div>
        <CollapsibleItem
          ...
          :text="product.text"
          :rate="product.rate"
          :defaultOpen="product.isNew || false"
          @close="() => handleRemoveProduct(index)"
        >
          <!-- Product Details Form -->
          <ProductDetailForm :product="product" :index="index" />
        </CollapsibleItem>
      </div>
